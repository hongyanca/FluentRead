...
<template>
  ...
  >
    ...
    <el-row v-show="service === services.localLlama" class="margin-bottom margin-left-2em" data-local-llama-base-url>
      <el-col :span="12" class="lightblue rounded-corner">
        <el-tooltip
          effect="dark"
          content="填写 OpenAI Chat Completions 兼容地址，例如 http://192.168.1.50:11434/v1/chat/completions。服务必须监听局域网接口。"
          placement="top-start"
          :show-after="300"
        >
      ...
      <el-col :span="12">
        <el-input
          v-model="config.proxy[service]"
          aria-label="Local Llama Base URL"
          placeholder="例如：http://192.168.1.50:11434/v1/chat/completions"
        />
      </el-col>
    </el-row>

    <el-row v-show="compute.showProxy && service !== services.localLlama" class="margin-bottom margin-left-2em">
      <el-col :span="12" class="lightblue rounded-corner">
        <el-tooltip effect="dark" content="可选的代理地址；填写后，请求会优先发送到这里。" placement="top-start" :show-after="300">
          <span class="popup-text popup-vertical-left">代理地址<el-icon class="icon-margin"><InfoFilled /></el-icon></span>
        </el-tooltip>
      </el-col>
      <el-col :span="12"><el-input v-model="config.proxy[service]" placeholder="默认直连服务接口" /></el-col>
    </el-row>
