/**
 * @file src/core/config/customBody.ts
 *
 * 文件职责：解析和规范化用户为翻译服务配置的自定义请求体，保证合并时只接受普通对象而不会改变原始 payload。
 * 主要内容：提供 parseCustomBody、isValidCustomBody、mergeCustomBody 以及字符串映射归一函数，覆盖 JSON 字符串、对象输入、非法数组和字段映射等边界。 可核对的公开符号包括 CustomBody、parseCustomBody、isValidCustomBody、mergeCustomBody、isCustomBodyMapping、normalizeCustomBodyMapping。
